...
async def sync_rates_db(db: Session, user_id: Optional[int] = None):
    ...
    # Store USD Oficial as primary in SystemSettings for now
    settings = db.query(SystemSettings).first()
    if not settings:
        settings = SystemSettings(bcv_rate=rates.get("usd_oficial", 0.0))
        db.add(settings)
    else:
    ...
    audit = AuditLog(
        user_id=user_id,
        action="SYNC_RATES",
        description=f"Tasas sincronizadas: USD {rates.get('usd_oficial')}, Paralelo {rates.get('usd_paralelo')}, EUR {rates.get('eur_oficial')}"
    )
    db.add(audit)
    db.commit()
    return rates
